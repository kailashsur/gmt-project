...
const Onboarding1 = ({ back, next, onboarding }) => {
  ...
  return (
    <div
      className="relative w-full h-full flex flex-col  items-center p-10 bg-cover bg-center transition-all duration-500 ease-in-out"
        ...
        <div className="  w-[311px] h-[400px] rounded-[48px] flex flex-col items-center justify-between bg-[#FE8C00] text-white p-8">
          <div className=" relative flex flex-col items-center justify-center w-full">
            ...
            <div className=" my-4 flex justify-center items-center gap-1 transition-all duration-500 ease-in-out">
              <div
                className={`w-7 h-2 ${onboarding == 1 ? "bg-white" : "bg-[#C2C2C2]"} rounded-full transition-all duration-500 ease-in-out`}
              ></div>
              <div
                className={` w-7 h-2 ${onboarding == 2 ? "bg-white" : "bg-[#C2C2C2]"} rounded-full transition-all duration-500 ease-in-out`}
              ></div>
              <div
                className={` w-7 h-2 ${onboarding == 3 ? "bg-white" : "bg-[#C2C2C2]"} rounded-full transition-all duration-500 ease-in-out`}
              ></div>
            </div>

            {onboarding == 3 ? <CircularProgressBar /> : ""}
          </div>

          <div className=" text-sm font-semibold flex justify-between items-center w-full">
            {/* Skip */}
            <span onClick={next} className=" cursor-pointer select-none">
              Skip
            </span>

            <span>
              {/* Back */}
              <span onClick={back} className=" cursor-pointer mx-2 select-none">
                <svg

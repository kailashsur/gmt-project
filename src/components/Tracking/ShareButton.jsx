...
const ShareButton = ({ speed }) => {
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      ...
      alert('Web Share API is not supported in your browser. Copy the link manually: ' + link);
    }
  };
  ...
  return (
    <div className="flex  items-center mt-4 gap-4 justify-center transition-all duration-500 ease-in-out">
      ...
      {sharedLink && (
        <div className="mt-4" onClick={handelCopyLink}>
          ...
          <p className="text-white text-sm font-bold underline">{sharedLink}</p>
        </div>
      )}
    </div>
  );
